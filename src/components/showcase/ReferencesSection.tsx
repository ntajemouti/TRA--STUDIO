...
import { ShieldCheck, Award, Building2, Users, Star, CheckCircle2, Video, Sparkles } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {STATS.map((stat, idx) => (
            <div
              key={idx}
              className="p-5 sm:p-6 rounded-2xl bg-zinc-900/70 border border-zinc-800/90 shadow-xl flex flex-col justify-between space-y-2 text-center sm:text-left"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {REFERENCES.map((ref) => (
            <div
              key={ref.id}
              className="rounded-2xl p-6 bg-zinc-900/50 border border-zinc-800/80 hover:border-zinc-700 transition-all duration-300 flex flex-col justify-between space-y-4 shadow-lg text-left group"
            >
